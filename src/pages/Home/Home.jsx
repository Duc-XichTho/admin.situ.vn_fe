...
import {
	...
	createNewSettingPublic, getAllPageSectionDataSharePublic,
...
export default function Home() {
	...
	const [selectedTag1, setSelectedTag1] = useState(tag1Filter);
	const [selectedTag2, setSelectedTag2] = useState(tag2Filter);
	...
	const fetchData = async () => {
		const data = await getNoteChartDataPublic(`Home`);
		if (data.length > 0) {
			const newNote = data.find(e => e.chartTitle == `Home`);
	...
	const openTagModal = () => {
		setSelectedTag1(tag1Filter);
		setSelectedTag2(tag2Filter);
	...
	const tag1DisplayCount = isMobile ? 2 : 3;
	const tag2DisplayCount = isMobile ? 2 : 3;
		...
		<div className={css.homeContainer}>
			<div className={css.homeHeader} style={{ backgroundColor: '#f5f5f5' }}>
				...
				<span className={css.logo}
					  style={isMobile ? { margin: '20px 20px 20px 0px' } : undefined}>HONGKY.INFO</span>
				{!isMobile && (
			...
			<div className={css.homeBody}
				 style={isMobile ? { minHeight: 'auto', height: 'auto', overflow: 'visible' } : { height: '90%' }}>
				<div
					className={css.homeBodyContent}
					style={{
						display: 'flex',
				...
				>
					<div style={{
						...
						padding: isMobile ? '20px 20px 10px 20px' : '40px 20px',
						height: '100%',
						display: 'flex',
						...
						{isMobile ? (
							<>
								<div onClick={() => setShowTipTap(v => !v)}>

									<span className={css.gioiThieuTitleMobile}
										  style={{ flex: 1 }}>Giới thiệu chung</span>
									<span className={css.gioiThieuTitle} style={{ marginLeft: 8 }}> {showTipTap
										? <DownOutlined style={{ fontSize: 11.5 }} />
										: <RightOutlined style={{ fontSize: 11.5 }} />}</span>
								</div>
								{showTipTap && <TipTapHome dataTiptap={dataTiptap} />}
							</>
						) : (
							<div>
								<div className={css.gioiThieuTitle}>Giới thiệu chung</div>
								<TipTapHome dataTiptap={dataTiptap} />
							</div>
						)}
					</div>
					<div style={{
						...
						<div style={{ display: 'flex', alignItems: 'center', marginBottom: 24, gap: 100 }}>
							<div style={{
								...
								>
									<span style={{ padding: isMobile ? '0px 10px' : '0px 0px' }}>Các chủ đề</span>
									{(tag1List.length > tag1DisplayCount ? tag1List.slice(0, tag1DisplayCount) : tag1List).map(tag => (
										<Button
											key={tag}
											type={tag1Filter === tag ? 'primary' : 'default'}
											onClick={() => setTag1Filter(tag1Filter === tag ? null : tag)}
											className={`${css.tag1} ${tag1Filter === tag ? css.tag1Active : ''}`}
										>
											{tag}
										</Button>
									))}
									{tag1List.length > tag1DisplayCount && (
								...
								>
									<span style={{ padding: isMobile ? '0px 10px' : '0px 0px' }}>Phù hợp</span>
									{(tag2List.length > tag2DisplayCount ? tag2List.slice(0, tag2DisplayCount) : tag2List).map(tag => (
										<Button
											key={tag}
											type={tag2Filter === tag ? 'primary' : 'default'}
											onClick={() => setTag2Filter(tag2Filter === tag ? null : tag)}
											className={`${css.tag2} ${tag2Filter === tag ? css.tag2Active : ''}`}
										>
											{tag}
										</Button>
									))}
									{tag2List.length > tag2DisplayCount && (
										<Button onClick={openTagModal}>...</Button>
									)}
								</div>
							</div>
						...
						<div style={isMobile ? { padding: '5 10px' } : {
							overflowX: 'hidden',
							height: '100%',
							padding: '5 10px',
						}}>
								...
								{filteredData.map((item) => (
									<Col xs={24} sm={24} md={24} lg={24} key={item.id}>
										<Card
											...
											style={{
												padding: 0, // Remove Card's own padding if needed
												...
												title={
													<div style={{ display: 'flex', alignItems: 'center' }}>
														...
														<div style={{
															...
															<div className={css.nameFolder}>{item.name}</div>
															<div className={css.titleFolder}>{item.mo_ta}</div>
															...
															<div style={{ display: 'flex', gap: '14px' , marginTop : '5px' }}>
																{item.tag1 && <div className={css.tag1}
																				   style={{ fontSize: isMobile ? 11.5 : undefined }}>{item.tag1}</div>}
																{item.tag2 && <div className={css.tag2}
																				   style={{ fontSize: isMobile ? 11.5 : undefined }}>{item.tag2}</div>}
															</div>
														</div>
													</div>
												}
			...
			>
				<div style={{ marginBottom: 16 }}>
					<div style={{ marginBottom: 8, fontWeight: 500 }}>Chủ đề</div>
					<div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
						{tag1List.map(tag => (
							<Button
								key={tag}
								type={selectedTag1 === tag ? 'primary' : 'default'}
								onClick={() => setSelectedTag1(selectedTag1 === tag ? null : tag)}
								className={`${css.tag1} ${selectedTag1 === tag ? css.tag1Active : ''}`}
							>
								{tag}
							</Button>
						))}
				...
				<div style={{ marginBottom: 24 }}>
					<div style={{ marginBottom: 8, fontWeight: 500 }}>Phù hợp</div>
					<div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
						{tag2List.map(tag => (
							<Button
								key={tag}
								type={selectedTag2 === tag ? 'primary' : 'default'}
								onClick={() => setSelectedTag2(selectedTag2 === tag ? null : tag)}
								className={`${css.tag2} ${selectedTag2 === tag ? css.tag2Active : ''}`}
							>
								{tag}
							</Button>
						))}
				...
				<div style={{ textAlign: 'right' }}>
					<Button
						type='primary'
						onClick={() => {
							setTag1Filter(selectedTag1);
							setTag2Filter(selectedTag2);
