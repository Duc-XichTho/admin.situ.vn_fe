...
import { getContentPageDataByIdPublic } from '../../../../apis/public/publicService.jsx';
import WikiExam from '../Quiz/WikiExam.jsx';
import { MyContext } from '../../../../MyContext.jsx';

export default function DetailTipTapContentShare() {
	const { editor } = useEditor();
	const [currentUser, setCurrentUser, ] = useState(null);
	const { checkAdminPage, checkEditorPage, setCheckEditorPage } = useContext(MyContext);
	...
	const fetchData = async () => {
		const data = await getContentPageDataByIdPublic(itemID);
		setFileNotePad(data);
	};
	...
	const handleBack = () => {
		navigate(`/${page}/sectionShare/${sectionPageID}`);
	};

	const [openQuiz, setOpenQuiz] = useState(false);

	const handleOpemQuiz = () => {
		setOpenQuiz(true);
	};

	const handleCloseDoQuiz = () => {
		setOpenQuiz(false);
	};
		...
		<div className={css.main}>
			<div className={css.pageInfo}>
				<div className={css.pageTitle}>
					{/*<TipTap_Icon width={20} height={20} />*/}
					{/*<span className={css.nameElement}>{fileNotePad?.name}</span>*/}
				</div>
				<div className={css.info}>
					<div className={css.infoRight}>
						<div className={css.controlContainer}>
						{
							checkAdminPage &&	fileNotePad?.quiz?.quiz?.length > 0 &&
								<Button onClick={handleOpemQuiz}>
									Làm quiz
								</Button>
							}
			...
			<WikiExam open={openQuiz} handleCloseDoQuiz={handleCloseDoQuiz} />
		</div>
