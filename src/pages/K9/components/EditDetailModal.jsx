...
const EditDetailModal = ({
    ...
    const handleSave = async () => {
        ...
        setLoading(true);
        try {
            const response = await updateK9({ id: item.id, detail: detail });

            if (response) {
                message.success('Cập nhật detail thành công!');

                // Cập nhật item với detail mới
                const updatedItem = { ...item, detail };
                ...
                message.error('Có lỗi khi cập nhật detail!');
            }
        } catch (error) {
            message.error('Có lỗi khi cập nhật detail!');
        } finally {
        ...
        <Modal
            ...
            title={
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span>Chỉnh sửa Detail - {item?.title}</span>
                </div>
            }
            open={visible}
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto' }}>
                        <TextArea
                            ...
                            placeholder="Nhập nội dung detail..."
                            style={{
                                overflowY: 'auto',
                ...
                <div style={{
                    ...
                    <div
                        ...
                        dangerouslySetInnerHTML={{
                            __html: (() => {
                                if (!detail) return '<div style="color: #999; font-style: italic;">Nhập nội dung để xem trước...</div>';

                                const { processedText, latexBlocks } = preprocessLatex(detail);
