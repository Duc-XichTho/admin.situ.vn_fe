...
const NewsTab = ({
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        const settings = await getSettingByTypePublic('CATEGORIES_OPTIONS');
        if (settings?.setting) {
  ...
  useEffect(() => {
    ...
    if (!isMobile && newsItems && !expandedItem && newsItems.length > 0 && viewMode === 'list') {
  ...
  }, [newsItems, selectedProgram, expandedItem, isMobile, viewMode]);
  ...
  // Grid item click handler - reuse selectedItem
  ...
  }, [bookmarkFilter, readFilter, quizStatusFilter, selectedProgram, filters]);
  ...
  const scrollToSearchResult = (resultIndex) => {
    ...
    // Wait a bit for state update and DOM to be ready
    setTimeout(() => {
      ...
      // Use the stored node if available (from performSearchInRenderedContent)
      if (result.node) {
        try {
          ...
          // Find the actual scroll container - look for the div with overflow: auto that contains the content
          let scrollContainer = null;

          // First, try to find the parent div with overflow: auto that contains markdownContentRef
          ...
          while (current && current !== document.body) {
            ...
            // Check if it's the main content container (has height: 75vh)
          ...
          // If still not found, try modal body or content panel
          if (!scrollContainer) {
          ...
          // Get the bounding rect of the range relative to viewport
          const rect = range.getBoundingClientRect();

          if (rect.height === 0 && rect.width === 0) {
            // Range might be collapsed, try scrolling to the node's parent
          ...
          // Use scrollIntoView directly - it's more reliable
          // Find the best element to scroll (prefer block-level elements)
          let elementToScroll = range.startContainer.parentElement;

          // Try to find a better parent (p, div, li, etc.)
          while (elementToScroll && elementToScroll !== markdownContentRef.current) {
            ...
            if (['p', 'div', 'li', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'span'].includes(tagName)) {
              // Check if it's a meaningful block element
          ...
          if (elementToScroll) {
            ...
            // Also try to scroll the container if needed
          ...
          // Highlight temporarily with a more visible effect
          ...
          // Fallback: scroll to parent element
  ...
  useEffect(() => {
    ...
    const maxX = window.innerWidth - 350; // panel width
    const maxY = window.innerHeight - 100; // min height
